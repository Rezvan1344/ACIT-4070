...
class PassengerForm(FlaskForm):
	adult =IntegerField('adult' ,widget=html5.NumberInput(min = 0, max = 5), validators=[DataRequired()])
	student = IntegerField('student' ,widget=html5.NumberInput(min = 0, max = 5),  validators=[DataRequired()])
	retired = IntegerField('retired',widget=html5.NumberInput(min = 0, max = 5),  validators=[DataRequired()])
	child = IntegerField('child',widget=html5.NumberInput(min = 0, max = 5),  validators=[DataRequired()], )
...
def payment():
    ...
    if form.validate_on_submit():
    	...
    	if cart==sample_cart:
            ...
            msg.body = f'Your purchase is completed successfully. Tickets are sent to your {a.email}. '
            mail.send(msg)
            return f'Your purchase is completed successfully. The ticket is sent to your email. '
